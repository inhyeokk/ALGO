...
public class Prim {
	public static void main(String[] args) {
		...
		for (int i = 0; i < V-1; ++i) {
			...
			int index = -1; // 찾으면 위치 저장
			for (int j = 0; j < V; ++j) {
				// 일단 안골라진지 검사, key의 최소값을 기억
				if (!check[j] && key[j] < min) {
					index = j;
					min = key[j];
				}
			}
			// index: 선택이 안된 정점 중 key가 제일 작은 노드
			check[index] = true;
			// index 정점에서 출발하는 모든 간선에 대해서 key를 업데이트
			for (int j = 0; j < V; ++j) {
				// check가 안돼있으면서, 간선은 존재하고,그 간선이 key값보다 작다면, key값을 업데이트
